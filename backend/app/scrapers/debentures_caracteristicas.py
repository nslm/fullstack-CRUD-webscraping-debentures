...
async def scraper_caracteristicas():
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            ...
            df["Juros Criterio Novo - Taxa"] = df["Juros Criterio Novo - Taxa"].apply(lambda x:float(str(x).replace(',','.').replace('%','').strip())*10000 if str(x).lower() not in ('nan', 'na', 'none', '-', '') else None)
