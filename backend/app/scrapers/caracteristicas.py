    ...
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            resp = await client.get(
                "https://www.debentures.com.br/exploreosnd/consultaadados/emissoesdedebentures/caracteristicas_e.asp?tip_deb=publicas&op_exc=False",  
            ...
            df = pd.read_csv(excel_io, sep="\t", header=2, encoding='cp1252')
            ...
            df["Data de Vencimento"] = df["Data de Vencimento"].apply(lambda x:datetime.strptime(str(x), "%d/%m/%Y").strftime("%Y-%m-%d"))
            df = df[["Codigo do Ativo", "Empresa", "Data de Vencimento", "indice", "Juros Criterio Novo - Taxa"]]
            df.rename(columns={"Codigo do Ativo":"codigo", "Empresa":"emissor", "Data de Vencimento":"vencimento", "Juros Criterio Novo - Taxa":"taxa"}, inplace=True)
